"""
기하학 연산 유틸리티 (Merged: Original + Refactoring Adapters)

위치: pose_transfer/utils/geometry.py
"""
import numpy as np
import math
...
# =============================================================================
# [Original Functions] 사용자님의 원본 코드 (그대로 유지)
# =============================================================================
...
def apply_bone_transform(start: np.ndarray, direction: np.ndarray, length: float) -> np.ndarray:
    """본 변환 적용 (시작점 + 방향 * 길이)"""
    return start + direction * length

def calculate_centroid(keypoints: np.ndarray, scores: np.ndarray, threshold: float = 0.3) -> np.ndarray:
    """신뢰도 가중 중심점 계산"""
...
# =============================================================================
# [Refactoring Adapter] 리팩토링된 코드와의 호환성을 위한 추가 함수
# =============================================================================

def get_angle(p1: np.ndarray, p2: np.ndarray) -> float:
    """
    두 점 (p1 -> p2) 사이의 절대 각도를 계산 (도 단위, x축 기준)
    * Refactored BodyLogic에서 사용
    """
    # 입력이 리스트나 튜플일 경우 numpy 배열로 변환
    p1 = np.array(p1)
    p2 = np.array(p2)
...
def rotate_point_degrees(point: np.ndarray, angle_degrees: float, origin: Tuple[float, float] = (0, 0)) -> np.ndarray:
    """
    Degree 단위를 사용하는 회전 함수 (기존 rotate_point는 Radian 사용)
    """
    return rotate_point(point, np.array(origin), np.radians(angle_degrees))